...
- Бесплатные чаты в платный LLM не ходят, но могут ходить в свой собственный эндпоинт из `DOORMAN_FREE_LLM_URL`:
...
- `DOORMAN_OPENROUTER_API` - включает LLM-проверки через OpenRouter, для платных чатов
...
- `DOORMAN_FREE_LLM_DISABLE` - ID чатов через запятую, где отключены бесплатные LLM-проверки профилей и сообщений, например `-1001234567890,-1009876543210`; пустое значение не отключает проверки ни в одном чате
