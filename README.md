...
- `DOORMAN_EROTIC_AUTOBAN_ENABLE=1` - включает для платных чатов перепроверку эротики при оценке основной модели `openai/gpt-6-luna:floor` от 75% моделями `xiaomi/mimo-v2.6-flash` и `qwen/qwen3.8-omni-flash` с тем же промптом и автобан, если обе дают от 85%; по умолчанию выключен
