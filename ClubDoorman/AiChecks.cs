            ...
            async ct =>
            {
                ...
                try
                {
                    ...
                    var prompt =
                        ...
                        $"В NonPersonProbability ответь, с какой вероятностью профиль даже не притворяется человеком (нет имени и фотографии человека, но например кот или фигура из мультфильма это нормально), а сразу выглядит как бизнес-аккаунт или реклама\n" +
                    ...
                    if (response.Value1 != null)
                    {
                        ...
                        if (probability.EroticProbability < Consts.LlmLowProbability && probability.NonPersonProbability < Consts.LlmLowProbability && probability.SelfPromotionProbability < Consts.LlmLowProbability && probability.GamblingProbability < Consts.LlmLowProbability)
