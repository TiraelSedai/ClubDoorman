    ...
    public async ValueTask<SpamPhotoBio> GetAttentionBaitProbability(Telegram.Bot.Types.User user, string? messageText, Func<string, Task>? ifChanged = default)
    {
        ...
        try
        {
            ...
            var prompt = $"""
                ...
                • ВАЖНО: простые приветствия (привет, hello, hi, здравствуйте и т.п.) от подозрительных профилей - это тактика спамеров для "прощупывания" группы перед отправкой рекламы. Если профиль выглядит подозрительно И сообщение содержит только приветствие - ЗНАЧИТЕЛЬНО повышай вероятность спама!
