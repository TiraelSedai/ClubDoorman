...
public class AiChecks : IAiChecks
{
    ...
    public async ValueTask<SpamPhotoBio> GetAttentionBaitProbability(Telegram.Bot.Types.User user, string? messageText, Func<string, Task>? ifChanged = default)
    {
        ...
        try
        {
            ...
            // ФИКС: Включаем информацию о канале прямо в основные данные профиля
            if (userChat.LinkedChatId != null)
            {
                try
                {
                    ...
                    _logger.LogDebug("🤖 AI анализ профиля: добавлена информация о канале {ChannelName} для пользователя {UserId}", 
                        linkedChat.Title, user.Id);
                }
                catch (Exception e)
                {
            ...
            // ФИКС: Обновляем nameBioUser с информацией о канале
            nameBioUser = sb.ToString();

            // ФИКС: Проверяем случай ТОЛЬКО канал (без био и фото) - должен анализироваться
            if (userChat.Bio == null && userChat.Photo == null && userChat.LinkedChatId != null)
            {
                _logger.LogDebug("🤖 AI анализ профиля: у пользователя {UserId} есть ТОЛЬКО канал (без био и фото) - выполняем анализ", user.Id);
            }
            ...
            var prompt = $"""
                ...
                ОБЯЗАТЕЛЬНО анализируй привязанный канал если он есть!
                ...
                • подозрительные каналы: реклама услуг, заработок, эротика, продажи
            ...
            // УДАЛЯЕМ: старый код добавления канала как отдельного сообщения
            // Теперь информация о канале уже включена в основной промпт
