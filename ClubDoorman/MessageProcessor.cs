...
internal class MessageProcessor
{
    ...
    private async Task DeleteAndReportMessageForProfile(Message message, string reason, CancellationToken stoppingToken)
    {
        _logger.LogDebug("DeleteAndReportMessageForProfile");
        var admChat = _config.GetAdminChat(message.Chat.Id);
        ...
        var restoreKey = $"restore_{IdGenerator.NextBase62()}";
        _logger.LogDebug("DeleteAndReportMessageForProfile: generated restoreKey={RestoreKey}", restoreKey);
        var deletedInfo = new DeletedMessageInfo(
            restoreKey,
            message.Chat.Id,
            user.Id,
        ...
        var row = new List<InlineKeyboardButton>([
            new InlineKeyboardButton(Consts.BanButton) { CallbackData = $"banNoMark_{message.Chat.Id}_{user.Id}" },
            new InlineKeyboardButton(Consts.OkButton) { CallbackData = $"attOk_{user.Id}_{restoreKey}" },
            new InlineKeyboardButton(Consts.ApproveButton) { CallbackData = $"approve_{user.Id}_{restoreKey}" },
        ]);
