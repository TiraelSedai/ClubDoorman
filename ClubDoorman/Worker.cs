    ...
    private async Task HandleUpdate(Update update, CancellationToken stoppingToken)
    {
        ...
        if (message.Text?.Trim().ToLower() == "/start")
        {
            if (message.Chat.Type == ChatType.Private)
            {
                var about =
$"""
<b>👋 Привет! Я — GateTroitsBot</b>
<i>🤖 Антиспам-бот для Telegram-групп</i>
...
🔹 <b>Защита от спама:</b>
   • Капча при входе (60 сек)
   • Блэклист lols.bot (бан 4 часа)
   • ML-анализ сообщений

🔹 <b>Умные фильтры:</b>
   • Длинные/странные имена
   • Перебор эмодзи
   • Сообщения от каналов

🔹 <b>Система доверия:</b>
   После 3 обычных сообщений все фильтры автоматически отключаются
...
👤 <b>Новым участникам:</b>
   ...
   • Дождитесь проверки капчей

👑 <b>Администраторам:</b>
   • Дайте права через 3-4 дня после добавления
   • Бот не мешает старожилам
   • Все действия логируются

━━━━━━━━━━━━━━━

<b>💎 Дополнительные планы:</b>

💚 <b>Без рекламы</b> — всего <b>$5 навсегда</b> за отключение рекламы в одной группе

💎 <b>Премиум</b> — <b>$12/год</b> за отдельную копию бота с персональным ML-датасетом под ваши группы (до 100)

🌐 <a href="https://momai.dev/antispambot">Подробнее о всех планах</a>
