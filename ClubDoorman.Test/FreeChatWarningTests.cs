...
public class FreeChatWarningTests
{
    ...
    public void Warning_KeepsTheReasonAndTheUpsell()
    {
        ...
        Assert.That(
            warning,
            Is.EqualTo(
                $"Сообщение с подозрением на спам. Обещает 200$ в день{Environment.NewLine}{Environment.NewLine}Проверки в базовом режиме выполняются легковесными моделями и могут содержать ошибки. Для более точного анализа переходите на PRO тариф."
            )
        );
    }
